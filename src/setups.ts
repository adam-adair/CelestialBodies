...
export const stableOrbit = (
  ...
  const sun = new Sphere("sun", 1, 16, 0.01, null, null, textures[2], Red); // metersToAU(1.3927e9) to get the real diameter of the sun
  const planet = new Sphere(
    ...
    textures[3],
    Green
  ); //metersToAU(12742000) to get the real diameter of the earth but it's way too small to see relative to the sun
...
export const binaryStars = (
  ...
  const startSpeed = 0.05;
  const mass = 0.01;
  const sun1 = new Sphere(
    "sun1",
    1,
    16,
    mass,
    ...
    textures[2],
    Red
  ); // metersToAU(1.3927e9) to get the real diameter of the sun
  const sun2 = new Sphere(
    "sun2",
    1,
    16,
    mass,
