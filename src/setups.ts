...
export const randomSystem = (numObjects:number, textures: (HTMLImageElement | ProceduralTextureData)[]): void => {
...
export const randomPlanetSystem = (numObjects:number, textures: (HTMLImageElement | ProceduralTextureData)[]): void => {
  ...
  export const repeatableSystem = (textures: (HTMLImageElement | ProceduralTextureData)[]) => {
  for(let x = 0 ; x<2; x++){
    const mass = 3;
...
export const texturesDisplay = async (gl: WebGLRenderingContext, program: WebGLProgram, player: Mesh, textures: (HTMLImageElement | ProceduralTextureData)[]): Promise<Mesh> =>{
  ...
  new Sphere("Object1", 0.8, 12, 0, null,null, textures[0]).addToAttractors().translate(-1, -1, -1);

  new Sphere( "Object2", 1, 16, 0, null, null, textures[1]).addToAttractors().translate(2, 1, 0);
  new Sphere("Object2", 0.5,  5, 0, null, null, null, Green).addToAttractors().translate(-2, 1, -5);
  // objects[2].rotate(-2, 1, -5);

  //sand textured sphere
 new Sphere("Sand", 0.7,  16, 0, null, null, textures[2]).addToAttractors().translate(-2, 3.5, 0);

  //grass textured sphere
 new Sphere("grass", 0.7,  16, 0, null, null, textures[3]).addToAttractors().translate(2, -3.5, 0);

  //clouds textured sphere
  new Sphere("clouds", 0.7, 16, 0 ,null, null, textures[4]).addToAttractors().translate(-3, -3.5, 0);
  ...
  export const twoPlanets = (textures: (HTMLImageElement | ProceduralTextureData)[]) => {
    for(let x = 0 ; x<2; x++){
      ...
      const body = new Planet(`Planet ${x}`,size, precision, mass, velocity, acceleration, texture, color).addToAttractors().addToMovers();
      const positionX = x %2===0 ? x*5 : -x*5;
      const positionY = x %2===0 ? x*5: -x*5;
      const positionZ = x %2===0 ? x*5 : -x*5;
      body.translate(positionX,positionY,0);
